...
class GradoModelo extends Conexion
{
	...
	public function BuscarPersona($campo, $dato){
		switch ($campo) {
			case 'inicioInscripcion':
				$cond = "inicioInscripcion like ?";
				$dato .= '%';
				break;
			default:
				$cond = 1;
		...
		$sql = "SELECT * FROM `personasgrado` INNER JOIN personas ON personasgrado.idPersonas = personas.idPersonas INNER JOIN grado ON personasgrado.idGrado = grado.idGrado WHERE $cond";
		//sprint $sql;
	}
}
?>